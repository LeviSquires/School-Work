...
std::ostream& operator<<(std::ostream& os, const Matrix& m)
{
	...
	{
		os << "| ";
		for (unsigned column = 0; column < m._data[row].size(); ++column)
		{
			os << std::left << std::setw(12) << std::fixed << std::setprecision(6) << m._data[row][column];
		}
		os << "|" << std::endl;
	}
	return os;
...
Matrix::Matrix(unsigned rows, unsigned columns, double value)
{
	...
	std::vector<double> vec(columns, value);
...
double Matrix::DotProduct(const std::vector<double>& v1, const std::vector<double>& v2)
{
	if (v1.size() != v2.size())
	{
		throw std::domain_error("Vector sizes are not the same in DotProduct().");
	}
	double ret = 0;
...
Matrix Matrix::operator*(const Matrix& m) const
{
	...
	{
		throw std::domain_error("Dimensions are not the same for Matrix Multiplication.");
	}
