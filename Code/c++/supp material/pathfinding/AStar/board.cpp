...
Board::Board(unsigned width, unsigned height, double lowWeight, double highWeight, double equalWeight, double heuristicWeight): 
	...
	_heuristicWeight(heuristicWeight)
{
	...
	for(unsigned i = 0; i < gridHeight(); ++i)
    {
		for(unsigned j = 0; j < gridWidth(); ++j)
		{
			...
			double endRow = _endIndex / gridWidth();
			double endCol = _endIndex % gridWidth();
			double manhattan = (abs(i - endRow) + abs(j - endCol)) * 0.5;
			cell.setHeuristic(manhattan); //_heuristicWeight
		}
	}
	...
	_openList.push_back(Route(_startIndex, _squares[_startIndex].getHeuristic()));
	...
	if(_squares[rt.getTerminus()].canMoveRight())
    {
		...
		if(beenThereDoneThat(_closedList, newIndex) == false)
		{
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostRight(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostRight(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
    ...
    if(_squares[rt.getTerminus()].canMoveLeft())
    {
        ...
        if(beenThereDoneThat(_closedList, newIndex) == false)
        {
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostLeft(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostLeft(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
        }
    }
	...
	if(_squares[rt.getTerminus()].canMoveUp())
    {
        ...
        if(beenThereDoneThat(_closedList, newIndex) == false)
        {
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostUp(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostUp(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
        }
    }
    if(_squares[rt.getTerminus()].canMoveDown())
    {
		...
		if(beenThereDoneThat(_closedList, newIndex) == false)
		{
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostDown(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostDown(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
    ...
    if(_squares[rt.getTerminus()].canMoveDownRight())
    {
		...
		if(beenThereDoneThat(_closedList, newIndex) == false)
		{
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostDownRight(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostDownRight(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
    ...
    if(_squares[rt.getTerminus()].canMoveUpLeft())
    {
		...
		if(beenThereDoneThat(_closedList, newIndex) == false)
		{
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostUpLeft(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostUpLeft(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
    ...
    if(_squares[rt.getTerminus()].canMoveDownLeft())
    {
		...
		if(beenThereDoneThat(_closedList, newIndex) == false)
		{
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostDownLeft(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostDownLeft(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
    ...
    if(_squares[rt.getTerminus()].canMoveUpRight())
    {
		...
		if(beenThereDoneThat(_closedList, newIndex) == false)
		{
			...
			if(it != _openList.end())
			{
				...
				{
					*it = Route(rt, newIndex, _squares[rt.getTerminus()].getCostUpRight(), _squares[newIndex].getHeuristic(), _heuristicWeight);
				}
			}
			else
			{
				_openList.insert(_openList.begin(), Route(rt, newIndex, _squares[rt.getTerminus()].getCostUpRight(), _squares[newIndex].getHeuristic(), _heuristicWeight));
			}
...
{
	return (rt1.getRouteWeight() + rt1.getHeuristic()) < (rt2.getRouteWeight() + rt2.getHeuristic());
}
#endif
...
{
#ifdef CPP_11
     _openList.sort([] (const Route& rt1, const Route& rt2){return (rt1.getRouteWeight() + rt1.getHeuristic()) < (rt2.getRouteWeight() + rt2.getHeuristic());});
#else
     _openList.sort(compareRoutes);
#endif
}

Cell& Board::getCell(unsigned row, unsigned col) const
{
...
void Board::show(std::ostream& os) const
{
            ...
            */
            ...
            {            
                ...
                else
                {
                    ...
                    {
                       *streams[3] << std::fixed << std::setprecision(1) 
                                   << std::setw(4) 
								   << cell.getHeuristic() << " ";
    ...
    {
		ss << " " << getCoordinates(it -> getTerminus()) 
		   << ": (" << it -> getRouteWeight() 
		   << " + " << it -> getHeuristic() 
		   << ": " <<  it -> getHeuristic() + it -> getRouteWeight()
		   << ")";
    }
    ss << std::endl;
