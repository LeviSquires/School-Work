...
{
public:
    Board(unsigned width, unsigned height, double lowWeight, 
	      double highWeight, double equalWeight, double heuristicWeight);   
	~Board()
	{
		delete [] _squares;
	}
...
//    static const unsigned gridWidth() = 6;
    Cell& getCell(unsigned row, unsigned col) const;
